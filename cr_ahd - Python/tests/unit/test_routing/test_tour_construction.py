...
def test__tour_cheapest_insertion(inst_and_sol_gh_0_ass9_routed6):
    ...
    assert cns.CheapestPDPInsertion()._tour_cheapest_insertion(instance, solution, 0, 0, 2) == expected
    assert cns.CheapestPDPInsertion()._tour_cheapest_insertion(instance, solution, 1, 0, 7) == expected
    assert cns.CheapestPDPInsertion()._tour_cheapest_insertion(instance, solution, 2, 0, 12) == expected

    assert cns.CheapestPDPInsertion()._carrier_cheapest_insertion(instance, solution, 0, solution.carriers[
...
def test__tour_cheapest_insertion_2(inst_and_sol_gh_0_9_assigned):
    ...
    assert pdp_insertion._carrier_cheapest_insertion(instance, solution, 0, [0, 1, 2]) == (0, None, None, None)
    pdp_insertion._create_new_tour_with_request(instance, solution, 0, 0)
    # enforce insertion of request 2
    assert pdp_insertion._carrier_cheapest_insertion(instance, solution, 0, [2]) == (2, 0, 1, 4)
    pdp_insertion._execute_insertion(instance, solution, 0, 2, 0, 2, 3)
    assert pdp_insertion._carrier_cheapest_insertion(instance, solution, 0,
